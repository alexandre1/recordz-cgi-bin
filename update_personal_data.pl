...
my %SERVER = ();
my $the_key = "otherbla";
my $the_key1 = "otherbla2938478hafajdvvjbbcBJHHJgudsa";
...
sub updatePersonalData {
	...
	my $encryptedPassword = &string2hex(&RC4($user_password,$the_key1));
	...
	if ($first_name &
     ...
     $name &
	    ...
	    $mydb->sqlUpdate("personne","nom_utilisateur='$decrypted'",(
	    ...
	    ($VALUE{'first_name'},$VALUE{'name_per'},
	     ...
	     $VALUE{'email'})= $mydb->sqlSelect("prenom, nom,adresse,ville, npa, no_telephone, email",
...
#	if ($username) {
		...
		($VALUE{'first_name'},$VALUE{'name_per'},
		 ...
		 $VALUE{'email'},$VALUE{'user_password'})= $db->sqlSelect("prenom, nom,adresse,ville, npa, no_telephone, email,mot_de_passe",
    		...
    		open (FILE, "<$dir/personal_data.html") or die "cannot open file $dir/personal_data.html";
		my $decryptedPassword =  &RC4(&hex2string($VALUE{'user_password'}),$the_key1);
		my $menu = $lp->loadMenu();
