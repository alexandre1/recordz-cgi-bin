...
sub loadPromote {
    ...
    my $encrypted =&string2hex(&RC4($article,$the_key));
    my $cgi = CGI::Lite->new;
    # Retrieve existing cookies
    my $cookies = $cgi->parse_cookies;
    my $oldcookie = defined ($cookies) ? $cookies->{ID} : 'not set';                  
    my $RETURN_URL = "http://avant-garde.info/cgi-bin/ipn.pl?lang=$lang&article=$article&";

    my  ($montant)= $mydb->sqlSelect("prix","article","id_article = $article");	
