...
sub loadUpdateArticlePayPal {
	...
	my  ($user_name,$id_personne,$first_name,$name,$adress,$city,$npa,$email,$phone_number)=sqlSelect("nom_utilisateur,prenom,nom,adresse,ville,npa,email,no_telephone", "personne",
        ...
        my  ($montant)= sqlSelect("montant,ref_vendeur,condition_payement_$lang.nom,condition_livraison.nom,a_paye.quantite,$ref_condition_payement",
	    			    "article,personne,a_paye,condition_payement_$lang,condition_livraison_$lang",
				   "ref_article = id_article AND id_article = $article AND a_paye.ref_statut = '8' AND ref_condition_payement = id_condition_payement AND  ref_condition_livraison  =  id_condition_livraison AND id_condition_livraison <> 8 ");	
...
sub loadCalendrier {
	...
	open (FILE, "<$dir/search_category_advanced.html") or die "cannot open file $dir/search_category_advanced.htmll";    
	while (<FILE>) {	
	    s/\$LABEL{'([\w]+)'}/ exists $SERVER{$1} ? $SERVER{$1} : $1 /eg;s/\$LANG/$lang/g;
	    s/\$ARTICLE{'main_menu'}/$menu/g;
	    s/\$SELECT{'calendrier_cat'}/$sport_menu/g;
	    s/\$LINK{'add_deal'}/$LINK{'add_deal'}/g;
	    s/\$SELECT{'moto_subcat'}/$submenu/g;
	    s/\$OPTIONS{'categories'}/$cats/g;
	    s/\$ARTICLE{'index'}/$index/g; s/\$ARTICLE{'table'}/$table/g;
	    s/\$ERROR{'([\w]+)'}//g; s/\$SESSIONID/$session_id/g;
	    $content .= $_;	
...
sub loadSearchParfum {
	...
	my $category = $query->param("category");
	my $fab;my $subcat = $query->param("subcat");	
	if ($category) {
...
sub loadPreferences {
	my $content;
	open (FILE, "<$dir/preferences.html") or die "cannot open file $dir/preferences.html";    
	while (<FILE>) {	
	    s/\$LABEL{'([\w]+)'}/ exists $SERVER{$1} ? $SERVER{$1} : $1 /eg; s/\$LANG/$lang/g;
	    s/\$ERROR{'([\w]+)'}//g; s/\$SESSIONID/$session_id/g;
	    $content .= $_;	
