...
sub loadAnimal {
 ...
 loadLanguage();
   my $categories = $lp->getAnimalCategories();
	...
	my $index = loadAnimalIndex();#$tableArticle->loadAnimalIndex();;
...
sub loadAnimalIndex {
    ...
    my  ($c)= $db->sqlSelect("count(DISTINCT (id_article))","article,met_en_vente $from","ref_article = id_article AND ref_statut = '3' AND  enchere_date_fin > '$date% ' AND quantite > 0 $add $add2");		
    my $counter = $c;
	my $total = $c;
	...
	if (not defined $previous_page) {
		...
		$string .= "<a href=\"/cgi-bin/f.pl?subcat=$subcat&category=$category&amp;min_index=0&amp;max_index=40&amp;index_page=$index_page&amp;page=lingerie&amp;lang=$lang&amp;page=main&amp;session=$session_id&amp;min_index_our_selection=0&amp;max_index_our_selection=40\" ><-First page-></a>&#160;&nbsp;";				
	...
	if (($index_page -1) > 0) {
		...
		$string .= "<a href=\"/cgi-bin/f.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=lingerie&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page\" ><-Previous-></a>&#160;&nbsp;";				
	}
	for my $index ($min_index..$counter) {						
		$next_page = $min_index + 40;
		if ($index_page < $count_per_page) {
			if (($index_page % $count_per_page) > 0) {							
				$string .= "<a href=\"/cgi-bin/f.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=lingerie&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page&amp;index_page=$index_page\"\" ><-$index_page-></a>&#160;&nbsp;";								
	...
	$string .= "<a href=\"/cgi-bin/f.pl?lsubcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=lingerie&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;min_index=$min_index&amp;max_index=$max_index&amp;previous_page=$previous_page&amp;next_page=$next_page\" ><-Next-></a>&#160;&nbsp;";				      
    return $string;     
}
