...
sub loadTvVideo {
	...
	if (trim($subcategory) eq trim($SERVER{'tv'})) {
	    ...
	    $ecran_size = $lp->loadSearchLecteurProperties();
	} elsif (trim ($subcategory) eq trim ($SERVER{'enceintes'})) {
	    $ecran_size = $lp->loadSearchLecteurProperties();
...
sub loadIndex {
	...
	if (not defined $previous_page) {
		...
		$string .= "<a href=\"/cgi-bin/q.pl?subcat=$subcat&category=$category&amp;min_index=0&amp;max_index=40&amp;index_page=$index_page&amp;page=tv_video&amp;lang=$lang&amp;page=main&amp;session=$session_id&amp;min_index_our_selection=0&amp;max_index_our_selection=40&amp;min_index=0&amp;max_index=40\" ><-First page-></a>&#160;&nbsp;";				
	...
	if (($index_page -1) > 0) {
		...
		$string .= "<a href=\"/cgi-bin/q.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=tv_video&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page&amp;min_index=$min_index&amp;max_index=$max_index\" ><-Previous-></a>&#160;&nbsp;";				
	}
	for my $index ($min_index..$counter -1) {						
		$next_page = $min_index + 40;
		if ($index_page < $count_per_page) {
			if (($index_page % $count_per_page) > 0) {							
				$string .= "<a href=\"/cgi-bin/q.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=tv_video&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;min_index=$min_index&amp;max_index=$max_index\"\" ><-$index_page-></a>&#160;&nbsp;";								
	...
	$string .= "<a href=\"/cgi-bin/q.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=tv_video&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;min_index=$min_index&amp;max_index=$max_index&amp;previous_page=$previous_page&amp;next_page=$next_page\" ><-Next-></a>&#160;&nbsp;";				      
    return $string;     
}
