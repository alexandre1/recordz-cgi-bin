...
sub detailOther {
   ...
   loadLanguage();
    ...
    ($ARTICLE{'id_article'},$ARTICLE{'name'},$ARTICLE{'author'}, $ARTICLE{'label'},
     ...
     $ARTICLE{'adresse'},$ARTICLE{'npa'}, $ARTICLE{'ref_location_ou_achat'},$ARTICLE{'ref_departement'},$ARTICLE{'link_youtube'})= $db->sqlSelect("DISTINCT id_article,article.nom,
							    ...
							    $param_immobilier_to_add, link_youtube",
   ...
   if ($ref_categorie eq '38') {
    ...
    if ($ARTICLE{'ref_subcategorie'} eq '75' or $ARTICLE{'ref_subcategorie'} eq '76' or $ARTICLE{'ref_subcategorie'} eq '77') {
    ...
    s/\$$ARTICLE{'pochette'}/$$ARTICLE{'pochette'}/g;
    ...
    s/\$ARTICLE{'link_youtube'}/$ARTICLE{'link_youtube'}/g;
...
sub loadHistoriqueLastBuyersIndex {
	...
	$string .= "<a href=\"/cgi-bin/historic.pl?lang=$lang&amp;session=$session_id&page=historique&amp;article=$article&amp;min_index=$min_index&amp;max_index=$max_index&cat=$cat&depot=$depot\"  class=\"menulink2\" ><-$j-></a>&#160;&nbsp;";		
	$min_index += 40;	
    }
...
sub loadHistoriqueLastBuyersByIndex {
	...
	$string .= "<td align=\"left\" width=\"253px\" height=\"100px\" style=\"background-image:url(../images/table_article_decoration.jpg);background-position:right bottom;background-repeat: no-repeat;\">&nbsp;&nbsp;&nbsp;<a href=\"javascript:;\" onClick=\"Lvl_P2P('/cgi-bin/detail_dealer.pl?lang=$lang&amp;session=$session_id&page=profil_vendeur&username=$ARTICLE{'nom_utilisateur'}',true,0500)\">$ARTICLE{'nom_utilisateur'}</a><br/>&nbsp;&nbsp;&nbsp;$ARTICLE{'prix'}<br/>&nbsp;&nbsp;&nbsp;$ARTICLE{'date_enchere'}</td>";
	$i += 1;$j += 1;
...
sub loadHistoriqueByIndex {
	...
	#$string .= " <label>&nbsp;&nbsp;$ARTICLE{'genre'}</label>";
	my $de = $ARTICLE{'nom_utilisateur'};
	$string .= "<td align=\"left\" width=\"253px\" height=\"100px\" style=\"background-position:right bottom;background-repeat: no-repeat;\">&nbsp;&nbsp;&nbsp;<a href=\"javascript:;\" onClick=\"Lvl_P2P('/cgi-bin/dealer_profil.pllang=$lang&amp;session=$session_id&page=profil_vendeur&username=$ARTICLE{'nom_utilisateur'}',true,0500)\">$ARTICLE{'nom_utilisateur'}</a><br/>&nbsp;&nbsp;&nbsp;$ARTICLE{'prix'}<br/>&nbsp;&nbsp;&nbsp;$ARTICLE{'date_enchere'}</td>";
	$i += 1;$j += 1;
	if ($i eq 1) {$string .= "<td align=\"left\" width=\"10px\"></td>";$i = 0;}
	if ($j eq 2) {$string .= "</tr>";$string .= "<tr>";$string .= "</tr>";$string .= "<tr>";$string .= "</tr>";$string .= "<tr>";$j = 0;}
    }
