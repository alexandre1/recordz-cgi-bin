...
sub loadImmobilier {
   my $show_popup = $query->param("show_popup");
	...
	my $selectionIndex = loadImmobilierIndex();#$tableArticle->loadImmobilierIndex();
...
sub loadImmobilierIndex {
	...
	if (not defined $previous_page) {
		...
		$string .= "<a href=\"/cgi-bin/immo.pl?subcat=$subcat&category=$category&amp;min_index=0&amp;max_index=40&amp;index_page=$index_page&amp;page=immo&amp;lang=$lang&amp;session=$session_id&amp;min_index_our_selection=0&amp;max_index_our_selection=40&country_swiss=$country_swiss&amp;country_france=$country_france&amp;with_lang_french=with_lang_french&amp;with_lang_italian=$with_lang_italian&amp;with_lang_german=with_lang_german&amp;with_lang_english=$with_lang_english\" ><-First page-></a>&#160;&nbsp;";				
	...
	if (($index_page -1) > 0) {
		...
		$string .= "<a href=\"/cgi-bin/immo.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=collection&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page\" ><-Previous-></a>&#160;&nbsp;";				
	}
	for my $index ($min_index..$counter) {						
		$next_page = $min_index + 40;
		if ($index_page < $count_per_page) {
			if (($index_page % $count_per_page) > 0) {							
				$string .= "<a href=\"/cgi-bin/immo.pl?subcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=collection&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;previous_page=$previous_page&amp;index_page=$index_page&amp;previous_page=$previous_page&amp;index_page=$index_page\"\" ><-$index_page-></a>&#160;&nbsp;";								
	...
	$string .= "<a href=\"/cgi-bin/immo.pl?lsubcat=$subcat&amp;category=$category&amp;lang=$lang&amp;page=collection&amp;session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;min_index=$min_index&amp;max_index=$max_index&amp;previous_page=$previous_page&amp;next_page=$next_page\" ><-Next-></a>&#160;&nbsp;";				      
    return $string;     
}