...
sub loadUpdateArticlePayment {
    my $id_a_paye = $query->param("id_paye"); 
    our $article = $query->param("article");
    our $username;
    our $u = $query->param("user");
    our $decrypted =  &RC4(&hex2string($u),$the_key);
    our ($iban,$paypal) = $db->sqlSelect("iban,paypal_me","article,personne,met_en_vente","id_article = $article and id_article = ref_article AND met_en_vente.ref_vendeur = personne.id_personne");
    my  ($montant)= $db->sqlSelect("a_paye.montant",
			   "article,personne, a_paye",
			   "a_paye.ref_article = id_article AND a_paye.ref_acheteur = id_personne AND nom_utilisateur = '$decrypted' and a_paye.ref_statut = '8' and a_paye.ref_acheteur = id_personne AND id_a_paye = $id_a_paye");	
    

    my  ($paypal_url)= $mydb->sqlSelect("paypal_me","personne","nom_utilisateur = '$decrypted'");	
    my $paypal_url = $paypal_url + "/" + $montant + "/CHF";
	...
	while (<FILE>) {	
	    ...
	    s/\$PAYPAL_URL/$paypal_url/g;	    
	    s/\$ARTICLE{'paypal'}/$paypal/g;
	    s/\$ARTICLE{'montant'}/$montant/g;
	    s/\$u/$u/g;
