...
sub doSearchSport {
    ...
    my  $from = "article,categorie_libelle_langue,libelle";
    my  $where;
	...
	$string .= "<a href=\"/cgi-bin/dosearchlingerie.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=0&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max&max_index=40&subcategory=$subcategory\" ><-First-></a>";		
	...
	if (($index_page -1) > 0) {
		...
		$string .= "<a href=\"/cgi-bin/dosearchlingerie.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max&subcategory=$subcategory\" ><-Previous-></a>";		
	}
	for my $index ($min_index..$counter) {						
		$next_page = $min_index + 40;
		if ($index_page < $count_per_page) {
			if (($index_page % $count_per_page) > 0) {							
				$string .= "<a href=\"/cgi-bin/dosearchlingerie.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max&subcategory=$subcategory\" ><-$index_page-></a>";		
	...
	$string .= "<a href=\"/cgi-bin/dosearchlingerie.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max&subcategory=$subcategory\" ><-Next-></a>";		
	return $string;
