...
sub updateArticleStatutIsPayed {
    ...
    if ($decrypted) {
        my ($userID,$acheteurEmail) = $db->sqlSelect("id_personne,email", "personne", "nom_utilisateur = '$decrypted'");
        my ($vendeurID) = $db->sqlSelect("ref_vendeur", "met_en_vente", "ref_article = '$article'");
        $db->sqlUpdate("a_paye","ref_article  = $article AND ref_acheteur = $userID AND id_a_paye = $id_a_paye",			
        ...
        $db->sqlInsert("a_livre",			
          ...
          ref_vendeur  => "$vendeurID"
            ...
            header => [
                          ...
                          Subject => 'Vous avez reçu un payment',
                      ],
            body => "Vous avez reçu un payment pour l'article $article par l'acheteur $decrypted",);
            sendmail($email, { transport => $transport });

        print "Location: http://avant-garde.no-ip.biz/cgi-bin/my_auctions.pl?lang=$lang&page=myencheredeal&option=mybuytobuy&u=$u\n\n";
    }
