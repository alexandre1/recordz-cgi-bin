...
sub doSearchSport {
    ...
    my  ($d)=$db->sqlSelectTest("count(article.nom)", $from, $where ." AND ref_statut = '3'");
	...
	$string .= "<a href=\"/cgi-bin/dosearchsport.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=0&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max&max_index=40\" ><-First-></a>";		
	...
	if (($index_page -1) > 0) {
		...
		$string .= "<a href=\"/cgi-bin/dosearchsport.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max\" ><-Previous-></a>";		
	}
	for my $index ($min_index..$counter) {						
		$next_page = $min_index + 40;
		if ($index_page < $count_per_page) {
			if (($index_page % $count_per_page) > 0) {							
				$string .= "<a href=\"/cgi-bin/dosearchsport.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max\" ><-$index_page-></a>";		
	...
	$string .= "<a href=\"/cgi-bin/dosearchsport.pl?lang=$lang&amp;action=dosearchcollection&amp;min_index=$min_index&amp;session=$session_id&category=$category&price_min=$buy_price_min&price_max=$buy_price_max\" ><-Next-></a>";		
	return $string;
