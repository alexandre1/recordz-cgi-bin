...
sub loadPageUpdateStatutDelivered {
    ...
    ($ARTICLE{'id_personne'},$ARTICLE{'first_name'},$ARTICLE{'name'},$ARTICLE{'country'},$ARTICLE{'adresse'},$ARTICLE{'city'},$ARTICLE{'npa'},
     $ARTICLE{'phone_number'},$ARTICLE{'email'}) = $db->sqlSelect("id_personne,prenom,nom,pays,adresse,ville,npa,no_telephone,email","personne,a_livre",
                                                                  "id_a_livre = $id_a_livre and ref_acheteur = id_personne");
    ...
    s/\$LABEL{'first_name_label'}/$LABEL{'first_name_label'}/g;    
    s/\$LABEL{'country_label'}/$LABEL{'country_label'}/g;    
	s/\$u/$u/g;
	s/\$ERROR{'([\w]+)'}//g;
