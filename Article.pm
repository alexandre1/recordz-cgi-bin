...
$dir = "/home/var/www/avant-garde.info/";
$dirLang = "/home/var/www/avant-garde.info/lang";
$dirError = "/home/var/www/avant-garde.info/lang";
$imgdir=  "/home/var/www/avant-garde.info/upload";
$session_dir = "/home/var/www/avant-garde.info/sessions";
$action = $query->param('action');
$session_id = $query->param('session');
my $mydb = MyDB->new;
...
sub loadArticleSelection {
	...
	$string .= "<a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;page=main&session=$session_id&amp;min_index_our_selection=$min_index&amp;max_index_our_selection=$max_index&amp;country_swiss=$country_swiss&amp;country_france=$country_france&amp;with_lang_french=$with_lang_french&with_lang_german=$with_lang_german&amp;with_lang_italian=$with_lang_italian&amp;with_lang_english=$with_lang_english\"  class=\"menulink2\" ><-$j-></a>&#160;&nbsp;<img alt=\"\" src=\"../images/next2.gif\" height=\"1px\" width=\"13px\">";		
	$min_index += 40;	
    }
	if (($nb_page % 10) > 0) {
		$string .= "<br/>";
...
sub viewArticleSelectionByIndex {
    ...
    my  ($c)= $mydb->sqlSelectMany("DISTINCT article.nom,marque,prix, pochette,id_article,ref_categorie,article.lieu",
		       "article,met_en_vente",
		       "ref_article = id_article  AND met_en_vente.notre_selection = '1' AND ref_statut = '3' and met_en_vente.page_principale = 'on' AND article.quantite > 0 and article.enchere_date_fin > '$date $time' $add  ORDER BY id_article DESC LIMIT $index_start, $index_end");	
	...
	$string .= "<td align=\"left\" width=\"255px\" height=\"152px\" style=\"background-image:url(../images/table_article_decoration.jpg);background-position:right bottom;background-repeat: no-repeat; border-width:thin;\">&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"/cgi-bin/testConnexion.pl?lang=$lang&amp;session=$session_id&amp;action=detailother&amp;article=$ARTICLE{'id_article'}\" class=\"menulink\" >$ARTICLE{'name'}</a><br/>&nbsp;&nbsp;&nbsp;&nbsp;<img alt=\"\" src=\"$ARTICLE{'pochette'}\" $add3 alt=\"\"/><br/>&nbsp;&nbsp;&nbsp;&nbsp;$ARTICLE{'price'} francs suisse <br/></td>";
	$i += 1;
	$j += 1;
...
sub getLastEnchereurDetail {
	...
	$string .= "<td align=\"left\"><a href=\"/cgi-bin/dealer_profil.pli?lang=$lang&amp;session=$session_id&page=profil_vendeur&username=$ARTICLE{'nom_utilisateur_acheteur'}\" class=\"menulink\" >$ARTICLE{'nom_utilisateur_acheteur'}</a></label></td>";			
	return $string;
}
...
sub doDealerIndex {
	...
	my $n2 = $i %= 10;
	$string .= "<a href=\"/cgi-bin/dealer_profil.pl?lang=$lang&amp;session=$session_id&page=profil_vendeur&username=$dealer&enchere=$option&min_index=$min_index&max_index=$max_index\"  class=\"menulink2\" ><-$i-></a>&#160;&nbsp;";		
	$min_index += 40;	
    }
    return $string;
}
...
sub doDealerIndexed {
   ...
   my $string .= "<table width=\"500\" border=\"0\"><tr bgcolor=\"#DBE4F8\"><td align=\"left\">$SERVER{'image'}</td><td align=\"left\" >$SERVER{'article_name_label'}</td><td>$SERVER{'price'}</td></tr>";    
   my  ($c)= $mydb->sqlSelectMany("DISTINCT article.nom,id_article,pochette,prix","personne,met_en_vente, article","nom_utilisateur = '$dealer' AND met_en_vente.ref_vendeur = id_personne AND met_en_vente.ref_article = article.id_article AND article.enchere = '$option' AND article.vendu = '$vendu' LIMIT $index_start, $index_end");
	 while( ($ARTICLE{'nom'},$ARTICLE{'id_article'},$ARTICLE{'pochette'},$ARTICLE{'prix'})=$c->fetchrow()) {
		$string .= "<tr><td align=\"left\"><img alt=\"\" src=\"$ARTICLE{'pochette'}\"></td><td align=\"left\"><a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;action=detailother&amp;article=$ARTICLE{'id_article'}\" class=\"menulink\" >$ARTICLE{'nom'}</a></td><td>$ARTICLE{'prix'}</td></tr>";
	}
    $string .="</table>";    
    return $string;
}
...
sub loadCommentaireIndex {
	...
	$string .= "<a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;min_index=$min_index&amp;max_index=$max_index&action=detailother&article=$article&viewcommentaire=1\"  class=\"menulink2\" ><-$j-></a>&#160;&nbsp;";		
	$min_index += 40;	
    }
...
sub getSoulevementCounter {
    ...
    my  ($c)= $mydb->sqlSelectMany("distinct id_article",
			   "article,a_livre,personne",
			   			   "ref_article = id_article AND ref_vendeur = id_personne  AND nom_utilisateur = '$username' and a_livre.ref_statut = '9' and ref_condition_livraison = 9 AND a_livre.ref_article = article.id_article");	
...
sub loadTableByIndexMyBuy {
    ...
    $string = "<table style=\"border-width:thin; border-style:dotted;border-top-color:#94CEFA;border-right-color:#94CEFA;border-left-color:#94CEFA;  border-bottom-color:#94CEFA\" width=\"555\" border=\"0\"><tr bgcolor=\"#DBE4F8\"><td align=\"left\" width=\"51\">$SERVER{'image'}</td><td align=\"left\" width=\"151\">$SERVER{'article_name_label'}</td><td align=\"left\">$SERVER{'buyer'}</td><td align=\"left\">$SERVER{'article_price_label'}</td><td align=\"left\">$SERVER{'article_quantity_label'}</td><td align=\"left\"></td></tr>";
    my  ($c)= $mydb->sqlSelectMany("DISTINCT a_livre.ref_acheteur,pochette,article.nom,id_article,id_a_livre,a_livre.quantite,a_livre.montant",
	...
	#$string .= " <label>&nbsp;&nbsp;$ARTICLE{'genre'}</label>";
	$string .= "<tr><td align=\"left\"><img alt=\"\" src=\"$ARTICLE{'pochette'}\" alt=\"\"/></td><td align=\"left\"><a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;action=detailother&amp;article=$ARTICLE{'id_article'}\" class=\"menulink\" >$ARTICLE{'name'}</a></td><td align=\"left\">$achteur</td><td align=\"left\">$ARTICLE{'montant'} francs suisse</td><td align=\"left\">$ARTICLE{'quantity'}</td><td align=\"left\"><a href=\"javascript:void(0)\" class=\"menulink\"  onclick=\"window.open('/cgi-bin/recordz.cgi?lang=$lang&amp;page=update_statut_article_waiting_buy&amp;session=$session_id&article=$ARTICLE{'id_article'}&buyer=$achteur&id_a_paye=$ARTICLE{'id_a_paye'}&u=$u;','MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=800,height=300,left=20,top=20')\">Update statut</a></td></tr>";
    }
    $string .= "</table>";
    return $string;
...
sub showArticleByIndex {
	...
	my  ($achteur)=$mydb->sqlSelect("nom_utilisateur", "personne", "id_personne = '$ARTICLE{'ref_acheteur'}'");	
	#$string .= " <label>&nbsp;&nbsp;$ARTICLE{'genre'}</label>";
	$string .= "<tr><td align=\"left\"><img alt=\"\" src=\"$ARTICLE{'pochette'}\" alt=\"\"/></td><td align=\"left\"><a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;action=detailother&amp;article=$ARTICLE{'id_article'}\" class=\"menulink\" >$ARTICLE{'name'}</a></td><td align=\"left\">$achteur</td><td align=\"left\">$ARTICLE{'montant'} francs suisse</td><td align=\"left\">$ARTICLE{'quantity'}</td><td align=\"left\"></td></tr>";
    }
    $string .= "</table>";
    return $string;
...
sub showMyDealPassedTable {
    ...
    $string = "<table style=\"border-width:thin; border-style:dotted;border-top-color:#94CEFA;border-right-color:#94CEFA;border-left-color:#94CEFA;  border-bottom-color:#94CEFA\" width=\"555\" border=\"0\"><tr bgcolor=\"#DBE4F8\"><td align=\"left\" width=\"51\">$SERVER{'image'}</td><td align=\"left\" width=\"151\">$SERVER{'article_name_label'}</td><td align=\"left\">$SERVER{'buyer'}</td><td align=\"left\">$SERVER{'article_price_label'}</td><td align=\"left\"></td></tr>";
	...
	my  ($achteur)=$mydb->sqlSelect("nom_utilisateur", "personne", "id_personne = '$ARTICLE{'ref_acheteur'}'");	
	#$string .= " <label>&nbsp;&nbsp;$ARTICLE{'genre'}</label>";
	$string .= "<tr><td align=\"left\"><img alt=\"\" src=\"$ARTICLE{'pochette'}\" alt=\"\"/></td><td align=\"left\"><a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;action=detailother&amp;article=$ARTICLE{'id_article'}\" class=\"menulink\" >$ARTICLE{'name'}</a></td><td align=\"left\"><a href=\"/cgi-bin/recordz.cgi?lang=$lang&amp;session=$session_id&amp;page=profil_vendeur&amp;username=$achteur\" class=\"menulink\" >$achteur</a></td><td align=\"left\">$ARTICLE{'montant'} francs suisse</td><td align=\"left\"></td></tr>";
    }
    $string .= "</table>";
    return $string;
